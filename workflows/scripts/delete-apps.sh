...
# tpg-delete-apps, one cluster. Applications for the cluster come from P_APPS (JSON map):
...
if [[ "$DRY" == "true" ]]; then
  for i in $targets; do
    ...
    record "result.${C}.${i}" SUCCEEDED DRY_RUN "would delete Postgres ${i} (fleet.yaml entry: ${in_git}; finalBackup=${P_FINAL_BACKUP:-true}, purgePvcs=${P_PURGE_PVCS}, purgeNamespace=${P_PURGE_NS})"
  done
